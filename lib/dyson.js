...
const installMiddleware = (app, options) => {
  ...
  if (options.bodyParserJsonLimit) {
    bodyParserOptions.limit = options.bodyParserJsonLimit;
  }
  if (options.bodyParserJsonStrict !== undefined) {
    bodyParserOptions.strict = options.bodyParserJsonStrict;
  }

  const bodyParserText = options.bodyParserText ? options.bodyParserText : {};
